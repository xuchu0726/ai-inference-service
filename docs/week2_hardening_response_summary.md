...
本文档用于汇总 Week2 性能优化实验后的补充验证工作，重点说明长上下文推理、FP8 KV Cache、量化路径、代码生成验证、显存统计口径、GSM8K 精度回归和环境复现边界的最终结果。

本文档只记录已经完成的实验事实、当前证据和仍存在的边界，不对未实测内容作推断。文档中的结论以仓库中已提交的 evidence、results、logs、scripts 和 docs 为依据。
...
但在单个 500K near-limit 请求中，FP8 KV 的端到端 latency 高于 BF16 KV。因此，当前实验只能说明 FP8 KV 对长上下文容量和 serving headroom 有明显帮助，不能表述为单请求 latency 优化。
...
当前项目已经完成 W8A8 compressed-tensors 路径的离线量化、checkpoint 持久化检查、vLLM serving、smoke test、并发压测和 GSM8K 精度回归。其他量化路径需要按实际结果区分，不能与 W8A8 stable path 混写。

### 3.1 已完成的 W8A8 compressed-tensors 路径
...
模型权重文件没有提交到 Git 仓库。仓库只保存 checkpoint 持久化检查记录、serving 日志、benchmark 结果和复现实验脚本。
...
### 3.2 历史 W8A8 serving 与性能对比证据

早期实验已完成 W8A8 serving、smoke test、batch profile 并发压测和 FP32/W8A8 batch profile 对比。该部分结果仍然有效，但需要与最新 clean volume checkpoint、serving 和 GSM8K 精度回归结果合并解释。
...
- 最新 GSM8K 精度回归进一步补齐了此前缺少的 accuracy 对比。
...
### 3.3 严格 INT8 / AWQ / GPTQ / compressed-tensors 直接 serving 失败边界
...
INT8、AWQ、GPTQ 等其他量化路径当前只作为兼容性边界和失败日志记录，不应表述为已完成稳定 serving。
...
需要说明的是，该评测不是官方 HumanEval 或 MBPP 完整 benchmark，只是轻量函数生成验证。
...
模型加载显存下降说明 W8A8 降低了权重加载阶段的 GPU memory footprint。运行时总显存不应按 `nvidia-smi` 占用量简单比较，因为 serving engine 会根据显存预算分配 KV Cache 和运行时缓冲区。对 serving 系统而言，更合理的收益表述是：W8A8 降低权重占用后释放了更多 serving headroom，可用于更长上下文、更高 batch/token capacity 或更稳的并发配置。
...
## 6. GSM8K baseline 与 W8A8 精度回归

### 6.1 BF16/vLLM baseline
...
- FP8 KV Cache 已验证可显著提升 KV Cache 容量和 512K 请求并发余量，但不能表述为单请求 latency 优化；
...
后续报告、简历和面试中建议采用以下表述口径。

### 9.1 长上下文与 FP8 KV Cache

可以表述为：

> 在 4×A100-SXM4-80GB 环境下完成 Seed-OSS-36B-Instruct 512K serving 配置真机验证，并完成约 500K prompt tokens 的 near-limit 请求测试；进一步启用 FP8 KV Cache，将 vLLM 日志中的 GPU KV cache size 从 909,360 tokens 提升至 1,807,008 tokens，使 512K 请求的 theoretical concurrency 从 1.73x 提升至 3.45x。

不应表述为：

> FP8 KV Cache 一定降低单请求 latency。

### 9.2 W8A8 量化与显存

可以表述为：

> 完成 Seed-OSS-36B-Instruct W8A8 compressed-tensors 离线量化、vLLM serving 和 full GSM8K 精度回归；W8A8 将模型加载显存从 67.59 GiB 降至 17.71 GiB，降幅为 73.8%。

不应表述为：

> W8A8 使运行时 GPU 总显存下降 73.8%。

### 9.3 GSM8K 精度回归

可以表述为：

> 在相同 GSM8K test split、相同评测脚本和相同生成参数下，W8A8 compressed-tensors profile 的 accuracy 为 74.7536%，BF16/vLLM baseline 为 75.7392%，绝对下降 0.9856 个百分点，API error rate 为 0.0%。
...
> 完成 50 题 HumanEval/MBPP-style 轻量代码生成验证，生成结果通过本地 Python 单元测试的比例为 52.0%，用于验证 serving、输出解析和基础代码生成链路。

不应表述为：

> 完成官方 HumanEval/MBPP benchmark。
