...
本报告记录当前 LLM 推理服务在本地真实模型后端下的初步 benchmark 结果。
...
4. latency 统计
5. tokens/s 统计
6. benchmark 结果 CSV 落盘
7. 实验结果文档化
```

这一步是项目从 mock backend 进入真实 TransformersBackend 的关键节点。
...
当前阶段中，`thinking_budget` 已经完成 API 参数传递和 benchmark 记录，但尚未真正映射到模型内部 reasoning token 控制机制。因此本实验主要验证参数链路、结果记录和真实模型推理能力。
...
### 8.3 client latency 与 server latency 的差异可以反映服务额外开销
...
### 8.4 当前 thinking_budget 尚未真正影响模型行为
...
```text
本地真实模型推理服务的初步 benchmark 报告
```

---

## 11. 与 PTA 第 1 周任务的对应关系
...
| 性能测试 | 已完成初步 latency / tokens/s 记录 |
| API 文档 | 已有基础文档 |
...
```text
1. 实现 P50 / P95 latency 统计
2. 实现 benchmark_concurrency.py
...
当前阶段已经完成从 mock backend 到真实 TransformersBackend 的升级。
...
这标志着项目已经从 toy-level mock demo 进入真实 LLM inference service prototype 阶段。

下一步应继续推进：

```text
P50 / P95 latency 统计和并发 benchmark。
```