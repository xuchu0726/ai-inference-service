...
> **历史阶段记录（2026-06-24）**：本文保留早期量化路线规划。当前量化事实以 `docs/week2_quantization_feasibility_report.md`、`docs/week2_quantization_protocol_audit.md`、`docs/week2_quantization_route_selection_matrix.md` 和主报告为准。W8A8 为稳定主 serving 路线；AWQ-Marlin 已完成独立 serving-stack 验证；BnB INT8 runtime 的完整 GSM8K 质量评测已完成；compressed-tensors strict INT8 stable vLLM serving 与 GPTQ 未完成。
...
1. 评估 Seed-OSS 的 INT8 量化可行性；
2. 分析 FP32 精度下的速度与显存对照边界；
3. 设计量化前后性能对比表；
4. 将显存降低 ≥30%、速度提升 ≥20% 作为后续真实量化实验的验证目标，不在未实测时声明达成。
...
4. 当前阶段不能虚假宣称 FP32 已完成，必须用显存估算和运行证据说明资源边界。
...
FP32 用于原始目标中的精度对照，但当前资源可能不足以直接 serving。
...
|---|---|---:|---:|---:|---:|---:|---:|---|---|
| BF16 baseline | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | baseline | baseline | 后续量化实验补充 | 实际主基线 |
| FP32 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 资源不足则写估算 |
| INT8 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 目标 ≥30% | 目标 ≥20% | 后续量化实验补充 | 优先尝试 |
| FP8/AWQ/GPTQ | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 后续量化实验补充 | 替代方案 |
...
本阶段量化路线的原则是：最大化尝试，但不虚假宣称。

当前最可靠的实际 baseline 是 BF16。FP32 是原始目标指定对照，但在 Seed-OSS-36B 与 2×A100 80GB 条件下大概率存在资源瓶颈。INT8/FP8/AWQ/GPTQ 是当前阶段需要优先验证的优化方向。

后续真实量化实验不能只写“量化可行”，而应给出可复现命令、运行结果、显存/延迟/吞吐对比、失败日志和资源边界说明。
